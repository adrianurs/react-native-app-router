...
function generateImports() {
  ...
  for(const exportItem of generated.exports) {
    fileContent = fileContent.concat(`'${Object.keys(exportItem).at(0)}': ${Object.values(exportItem).at(0)},\n`);
  }

  fileContent = fileContent.concat('}\n');

  generateFile(OUTPUT_FILE, fileContent).then(() => {
    console.log('react-native-app-router: imports.js generated successfully.');
  }).catch((err) => {
...
function generateStatementsForRoute(route) {
  ...
  if (route.layoutFile) {
    ...
    imports.push(`import ${component} from '../../../../src/${route.layoutFile}'`);
    exports.push({[route.layoutFile]: component});
  }

  imports.push(`import ${normalizedComponentName} from '../../../../src/${route.screenFile}'`);
